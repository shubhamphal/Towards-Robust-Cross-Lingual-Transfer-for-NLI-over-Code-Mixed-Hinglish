...
class LSTM_Model():
    def __init__(self, input_shape, lr, a_dim, t_dim, emotions, attn_fusion=True, unimodal=False,
                 enable_attn_2=False, seed=1234):
        print('input_shape: ', input_shape)
        if unimodal:
            self.input = tf.placeholder(dtype=tf.float32, shape=(None, input_shape[0], input_shape[1]))
        else:
            self.a_input = tf.placeholder(dtype=tf.float32, shape=(None, a_dim))
            self.t_input = tf.placeholder(dtype=tf.float32, shape=(None, t_dim))
    ...
    def self_attention(self, inputs_a, inputs_t, name):
        """
    ...
    def _build_model_op(self):
        ...
        if self.unimodal:
            ...
            if self.attn_fusion:
                input = self.self_attention(self.a_input, self.t_input, '')
                input = input * tf.expand_dims(self.mask, axis=-1)
            else:
                input = tf.concat([self.a_input, self.t_input], axis=-1)
